...
class KimiAdapter extends adapter_sdk_1.BaseAdapter {
    constructor() {
        super(...arguments);
        this.name = 'kimi';
        this.version = '1.0.0';
    ...
    startKimiProcess() {
        ...
        this.kimiProcess = (0, child_process_1.spawn)(kimiPath, args, {
            cwd: workDir,
    ...
    handleKimiRequest(requestId, params) {
        // 处理 Kimi 的请求（如审批请求）
    ...
    async handleApprovalResponse(command) {
        const { requestId, decision, feedback } = command.payload;
        if (!requestId || !this.kimiWs)
            return;
        // 构造 Kimi 格式的响应
        const response = {
            jsonrpc: '2.0',
    ...
    async handleCancelTask(command) {
        if (!this.kimiWs || !this.sessionId)
            return;
        // 发送取消命令到 Kimi
        const cancelMsg = {
            jsonrpc: '2.0',
